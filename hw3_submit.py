# -*- coding: utf-8 -*-
"""hw3_submit_colab.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1y6kJCsAnkCKOAzN-rSAeeIc5g9BVilA7
"""
...
def load_params(ShallowCNN, file_name="gdrive/My Drive/AI_data/params3rd.pkl"):
...
class ShallowCNN:
    ...
    def backpropagation(self, learning_rate): 
        ...
        x = self.layers['F8'].backward(x)
        x = self.layers['R7'].backward(x)
        x = self.layers['F7'].backward(x)
        x = self.layers['R6'].backward(x)
        x = self.layers['F6'].backward(x)
        x = self.layers['P5'].backward(x)
        x = self.layers['R4'].backward(x)
        x = self.layers['C4'].backward(x)
        x = self.layers['P3'].backward(x)
        x = self.layers['R2'].backward(x)
        x = self.layers['C2'].backward(x)
        x = self.layers['R1'].backward(x)
        x = self.layers['C1'].backward(x)
        '''
        모든 layer에서 backword를 진행하여 gradient를 구한다.
        '''
        grads = {}
        ...
        '''
        각 층에서 구한 W,b의 gradient를 하나의 배열에 저장한다. 
        '''
